...
| **Independent run** | The isolated planned unit of one simulated world executed under one registered configuration, and the unit of isolated concurrent execution that WP2-SCOPE-001 and WP2-SCOPE-003 govern. It is not one physical execution: it is the isolated planned unit whether it has zero, one, or several execution attempts. An independent run is declared as a planned run and may be realized through zero or more execution attempts. A planned run that is pending, cancelled, rejected, or otherwise never launched remains visible as an independent run with zero execution attempts and no authoritative accepted outcome. Once execution is attempted, every execution attempt is separately preserved, and every one has its own run evidence root. An independent run produces at most one authoritative accepted outcome, reached only if some execution attempt's result is accepted and then fixed at that single attempt's authoritative commit. | New term at the specification level, consistent with charter Section 10.1's statement that "Independent runs are separate simulated worlds with no shared mutable state". The repository already uses the bare word *run* in a distinct sense — the model-backed browser run, whose `runId` is minted by `src/app/modelGatewayClient.ts::ModelGatewayClient` in `newRun`, carried on the wire by `src/sim/decisions/externalSchemas.ts::externalDecisionRequestEnvelopeSchema`, and used by `gateway/server.ts::createGateway` to key its per-run budget and idempotency state. That meaning is stable and is preserved here, not redefined. This specification does not use the bare word *run* as a defined term: it writes *independent run* for the concurrency unit defined here, *planned run* and *execution attempt* for the planning and execution senses, and *model-backed browser run* — or *model run* — for the repository's `runId` sense; where the bare word appears in Section 5 it is ordinary description of a cited repository mechanism rather than a use of a defined term, and WP2-SCOPE-005 binds only those qualified forms. |
...
| **Execution attempt** | One physical execution of one planned run, producing its own evidence in its own run evidence root and receiving exactly one terminal disposition. Several execution attempts may belong to one planned run, and therefore to one independent run, and a planned run that is never launched has none. | Corresponds to the repository's *execution* (`scripts/experiments/m2/sequenceState.ts::attemptExecutionSchema`, field `executionId`, minted by `nextExecutionId`). Section 5.14 records the conflict between this implemented distinction and the Milestone 2 brief's wording. |
...
Each register entry states the same eight attributes in the same order. The register supplies normative entity-model definitions, which WP2-ID-027 in Section 6.16 incorporates and makes binding, except where an entry expressly assigns a question to a later section.
...
**Rationale.** Admission is the act that creates an execution-attempt identity, and admission is not successful launch, running, or completion: an attempt that is admitted and then cancelled, or that fails before any execution activity begins, keeps the identity its admission created together with the disposition it in fact reached, consistent with charter Section 14.4's requirement that failed, interrupted, invalid, excluded, and replacement attempts remain recorded evidence with typed dispositions. Section 3.1's defining sentence for the execution attempt — one physical execution of one planned run — states what an admitted execution attempt is: the identity names one specific physical execution, and this register states when that identity begins to exist, so an attempt cancelled or failed before any execution activity began remains the recorded identity of a physical execution that was admitted and never began. Three passages stated elsewhere describe the complementary case and are read here accordingly. Section 3.1's execution-attempt entry states that a planned run that is never launched has none; Section 3.1's independent-run entry states that a planned run that is pending, cancelled, rejected, or otherwise never launched remains visible as an independent run with zero execution attempts and no authoritative accepted outcome; and WP2-ID-007 states the same of a pending, cancelled, rejected, or otherwise never launched planned run. Each of them describes the case in which no execution-attempt identity was ever admitted for the planned run, which is exactly the case in which the count of that planned run's execution attempts is zero. Sections 1 through 5 are not revised by this chunk, and WP2-ID-007's wording is likewise carried unchanged, so the reading is recorded here rather than written into any of them, in the way Section 6.1 records the inconsistencies this document carries with itself. Lifecycle states and transitions, including what may occur between admission and the start of execution activity, are assigned to Section 8.
...
| Exists without physical execution | Yes. The execution attempts a dispatch wave groups are admitted execution attempts, and a wave every one of whose member attempts is cancelled after admission and before release, and before any execution activity begins, retains its identity while those member attempts keep the identities their admission created. |
...
**WP2-ISO-019.** The finalization state of an execution attempt **MUST** be held separately from the finalization state of every other execution attempt, including that of another execution attempt of the same planned run, and the planned run's authoritative-outcome record, where that outcome is reached, identify the one accepted execution attempt rather than be represented as mutable finalization state shared among execution attempts.
