...
Section 12.2 states how the model-request record model is read and carries the requirement that makes it binding. Section 12.3 holds the record register. Section 12.4 separates the three lifecycle layers and their stage events. Sections 12.5 through 12.7 state the logical-request, attribution, and idempotency requirements. Section 12.8 states the budget and usage contract. Section 12.9 states the provider-behavior requirements. Section 12.10 records options deliberately left unselected, and Section 12.11 records what is assigned to later chunks.
...
**Rationale.** Section 1.1 states, in the present tense and of the head at which it is read, that Sections 6 through 21 hold no substantive content; Section 4.2 states in the same form that the remaining eleven namespaces hold no assigned identifier at this head; the rationale of Section 6.1 records that the identifiers assigned there leave nine of the reserved namespaces unassigned and draws the operative conclusion that Sections 8 through 21 and the conformance document still hold no substantive content, which Chunk 4 falsified for Sections 8 through 11 and this chunk falsifies further; the rationale of Section 8.1 records that six of the reserved namespaces rather than nine remain unassigned; and that same Section 8.1 rationale draws the operative conclusion that Sections 12 through 21 and the conformance document still hold no substantive content. None of those statements is accurate at the head this chunk produces: Sections 12 and 16 now hold substantive content, identifiers are now assigned in the `WP2-MODEL-###` and `WP2-SEC-###` namespaces as well, and four of the reserved namespaces rather than six remain unassigned — `WP2-EVID-###`, `WP2-FAIL-###`, `WP2-AGG-###`, and `WP2-HAND-###`. This chunk revises no part of Sections 1 through 11, so none of those sentences can be corrected here; all of them are recorded as inconsistencies this document now carries with itself, in the way Section 5.14 records a carried item and the rationales of Sections 6.1 and 8.1 record their own, and all of them are left to the final integration chunk that the staged instructions assign, whose required work includes removing incomplete-section language. The operative conclusion those passages draw survives on independent grounds: Sections 13 through 15, Sections 17 through 21, and the conformance document still hold no substantive content, and Section 1.4 records that no established specification version exists, so the contract remains incomplete and cannot be treated as a settled technical baseline.
...
| Attribute | Statement |
| --- | --- |
| Identity scope | Every immutable request specification the laboratory records, across all studies and all concurrent execution: the canonical request-content reference or digest that fixes exactly what one logical model request asked. Its content and its reference or digest carry no secret material, which is the secret-material half of the request-content rule that Section 16 states. |
...
| Cardinality | One logical model request binds exactly one immutable request specification. A content reference or digest is never reused for different content, as WP2-MODEL-007 requires; two logical requests whose content is identical remain two requests, each binding its own specification record, however their content references compare. |
...
| Required parent and source references | The exact provider response it evaluated and the exact immutable request specification it evaluated that response against, as WP2-MODEL-010 requires. |
| Cardinality | One response validation evaluates exactly one provider response against exactly one immutable request specification. A provider response is validated zero or more times, each evaluation a separate record with its own verdict. |
...
| Required parent and source references | Exactly one logical model request; the validated response it accepts, where it is an acceptance, and the exact provider response it rejects or supersedes, where it is a rejection or a supersession; the deciding episode or decision it resolves; for the cancellation kind, the identity of the cancellation record of the Sections 8 and 9 cancellation authority whose effect reached the request — the recording of the resolution itself remaining with the deciding engine authority, the cancellation record being its triggering fact, consistent with WP2-MODEL-044; and, for the no-call kind, its reason and its deciding authority, as WP2-MODEL-016 requires. |
...
| Attribute | Statement |
| --- | --- |
| Identity scope | Every idempotency resolution the laboratory records: the typed resolution of one presentation of an already-known request identity, of exactly the kinds idempotent replay — answered from the recorded result — and identity/content conflict refusal. |
| Issuing or recording authority | The authority that resolves presentations of model requests; where a component serves model requests for more than one isolation unit, the recorded results it answers from are keyed as WP2-ISO-015 requires, so that one unit's presentation is never answered from another unit's recorded result. |
| Required parent and source references | The logical model request identity presented; the recorded result answered from, where the kind is idempotent replay; and both content identities — the recorded one and the conflicting one — where the kind is identity/content conflict refusal, as WP2-MODEL-026 requires. |
| Cardinality | One idempotency resolution resolves exactly one presentation. One logical model request accumulates zero or more idempotency resolutions over its history. |
| Point of existence | At the authority boundary at which the presentation is resolved, in the terms WP2-MODEL-024 fixes. |
| Correction or supersession rules | A refusal record is preserved as recorded — never deleted, overwritten, truncated, or rendered unverifiable, and carrying both content identities as WP2-MODEL-026 requires — the same evidence-preserving discipline WP2-ISO-031 and WP2-ISO-032 apply to the refusals WP2-ISO-030 requires; no idempotency resolution is rewritten to change its kind or its named identities, and a material recording error is corrected under WP2-ID-025 and WP2-ID-026. |
| May it exist without a provider call | Yes. An idempotent replay answers from a recorded result and makes no provider call, and an identity/content conflict refusal dispatches nothing. |
...
| Required parent and source references | The logical model request; the prior local dispatch attempt or provider attempt the retry or fallback proceeds from; the identity and version of the registered retry or fallback policy applied; and every changed model, provider, serving-provider, route, prompt, and policy identity, which WP2-MODEL-030 makes a condition of a fallback's preserving its originating request and which this entry requires of every retry or fallback it records. |
...
| Issuing or recording authority | The reconciling authority under the applicable registered budget policies. |
...
| Response validated | Local dispatch and provider attempt | The response-validated stage: one response validation of one response against one immutable request specification. | Not simulation acceptance, which WP2-MODEL-013 keeps engine-owned. |
| Response accepted by the simulation | Logical request and simulation resolution | The simulation-accepted stage: the engine-owned acceptance naming the validated response it accepts and the deciding episode or decision it resolves. | Not a consequence of provider success, local receipt, or validation alone. |
...
**WP2-MODEL-007.** A logical model request identity, and the immutable request-content reference or digest bound to it, **MUST NOT** be reused for different request content.
...
**WP2-MODEL-010.** Every response validation **MUST** name the exact provider response it evaluated and the exact immutable request specification it evaluated that response against.

**WP2-MODEL-011.** Every simulation acceptance **MUST** name the validated response it accepts and the deciding episode or decision it resolves.
...
**Rationale.** These invariants state the record-model half of rules whose other halves are already established, and they redefine none of them: WP2-MODEL-012 bounds the resolution count in the way WP2-ID-009 bounds the authoritative accepted outcome of a planned run — at most one, possibly never reached — and the engine ownership WP2-MODEL-013 protects is the authority charter Section 14.8 assigns, which Section 11.4 already keeps logical time under; WP2-MODEL-017 states what the record holds and leaves the timing rules themselves to WP2-TIME-010 and WP2-TIME-011; and WP2-MODEL-018 applies to request records the join discipline WP2-ISO-014 states between isolation units and WP2-TIME-016 states across hosts. WP2-MODEL-014 and WP2-MODEL-015 divide the late-response rule into its preservation half and its prohibition half so that each states one obligation on one subject: the late response is evidence — of provider behavior, of transport delay, and of the resolution race it lost — and evidence that changed simulation state silently would contradict the resolution record the engine already made authoritative. The engine-owned lifecycle facts Section 5.6 records about the current repository — the engine's resolved-request registry recording accepted, expired, and superseded resolutions — are evidence that such a resolution model is representable, not a requirement, and WP2-SCOPE-010 keeps them from being read as one.

### 12.6 Complete attribution

**WP2-MODEL-019.** Every logical model request, local dispatch attempt, provider attempt, provider response or failure result, response validation, simulation resolution, incurred usage or charge record, and budget authorization, settlement, or other budget event **MUST** be attributable, through explicitly recorded lineage, as applicable, to the study identity and version; the registration identity and version; the experiment identity and version; the condition and treatment identities; the planned-run identity; the execution-attempt identity; the laboratory-worker identity; the assignment-authority grant in exercise of which the operation was carried out, together with whether that grant was current for that operation, as the recorded authority facts of the Section 9.6 record model establish it; the originating cognitive episode or decision, and a specific decision where one exists; the decision authority; the prompt and context identity or version; the requested model identity; the configured provider, serving-provider, and route identities; the observed upstream provider and returned model identity, where observable; a nonsecret provider-project or account-scope identity, where applicable; the registered concurrency-profile identity and version; the applicable time-domain records; and every budget scope to which the operation is charged — naming each of those that applies and stating the absence of any that does not.
...
**WP2-MODEL-027.** A transport or delivery retry **MUST** retain the logical model request identity and the immutable request specification of the request whose delivery it repeats, receiving its own local dispatch attempt record where it is in fact dispatched rather than answered as an idempotent replay under WP2-MODEL-025 or refused under WP2-MODEL-026.
...
**WP2-MODEL-030.** A fallback **MUST** preserve the originating logical model request only where it continues to serve the same registered cognitive operation that request serves and its retry and fallback lineage record names every changed model, provider, serving-provider, route, prompt, and policy identity.
...
**Rationale.** These requirements answer the idempotency question the WP2-ISO-015 rationale assigns here — which rule applies to a genuine repeat of one request — and they answer it in three typed cases rather than one, each of them a delivery repetition inside the Section 3.1 term rather than outside it, the difference being how the presentation resolves, every presentation of an already-known request identity being classified and resolved at the race-safe authority boundary WP2-MODEL-024 fixes: a repeat with matching content of a request no authoritative terminal simulation resolution has yet resolved is dispatched again, keeping its identity and receiving its own local dispatch attempt record under WP2-MODEL-027, that further-delivery case being recorded by the further local dispatch attempt record it produces rather than by an idempotency-resolution record, which keeps the idempotency-resolution entry of the register in Section 12.3 to exactly the kinds it states; a repeat with matching content of a request an authoritative terminal simulation resolution has already resolved is answered from that recorded result as an idempotent replay under WP2-MODEL-025, dispatching nothing; and a repeat whose content conflicts is an identity/content conflict refused under WP2-MODEL-026, this section applying to that refusal the same evidence-preserving discipline WP2-ISO-031 and WP2-ISO-032 apply to the refusals WP2-ISO-030 requires, rather than adding a condition to WP2-ISO-030. What separates a retry from a new request is the decision served, not the transport history: WP2-MODEL-029 keeps a new decision from hiding inside an old identity, WP2-SCOPE-006 already prohibits recording a transport or delivery retry interchangeably with a registered replacement or a process restart, and WP2-SCHED-005 resolves the analogous repetition for assignment delivery. The fallback rules keep the treatment honest: a fallback that changes a model, route, or prompt within the registered policy is recorded lineage under WP2-MODEL-030, and one that would change what the study tests is an unregistered treatment change that WP2-MODEL-031 prohibits concealing, with WP2-SCHED-042 and WP2-SCHED-043 governing the divergence record where one arises. The disposition vocabulary for the preserved non-authoritative responses WP2-MODEL-032 requires is assigned to Section 14. WP2-MODEL-033 states the restart half of the durability rule, and WP2-MODEL-042 states the accounting half; Section 5.6 records that the repository's current per-run idempotency and budget state is process-local, evictable, and reset at every attempt boundary, which is a current limitation and, under WP2-SCOPE-010 and Section 5.14 item 5, establishes neither a requirement nor a permitted design.
...
### 12.11 Boundaries assigned to later chunks
...
**WP2-SEC-003.** A conforming laboratory **MUST** make secret material available only within the provider-dispatch secret boundary authorized to use it, and to no component, surface, or record outside that boundary.
...
### 16.10 Boundaries assigned to later chunks
