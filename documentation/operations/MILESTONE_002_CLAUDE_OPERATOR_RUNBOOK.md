...
- `attempt-*/trace-manifest.json` (Phase 4; evidence-instrumentation revision): the reviewed capture profile, every retained Playwright trace chunk with its size, the rotation cadence, the `retain-all-chunks` policy, and — under `semantic-trace-sparse-visual-v1`, which every FORMAL plan must declare — the hashed static visual checkpoints (`visual-checkpoints/*.png`: before Start, at every rotation, at completion; each with filename, UTC timestamp, logical tick, run status, and SHA-256). Formal traces carry NO continuous screenshot stream (~121 KB/min semantic rate at 1× versus ~163 MB/min with screenshots); the final/failure/stall screenshots and failure DOM snapshot are separate artifacts as before. Rotated chunks live in `attempt-*/trace-chunks/`; the final chunk is `attempt-trace.zip` (or `failure-trace.zip`). A projected evidence-budget overrun fails the attempt EARLY as `evidence-budget-exceeded: forecast …` — treat it as a real budget failure, never delete chunks to make room. Registered ceilings: Stage A 8 GiB, calibration 32 GiB.
